// Route
const MAIN_ROUTE = "/";
const MOVIES_ROUTE = "/movies";
const SAVED_MOVIES_ROUTE = "/saved-movies";
const PROFILE_ROUTE = "/profile";
const REGISTER_ROUTE = "/signup";
const LOGIN_ROUTE = "/signin";

// Error message
const ERROR_MESSAGE='Something went wrong! Try again.';
const SUCCESS_LOGOUT_MESSAGE='You have successfully logged out!';
const UPDATE_PROFILE_MESSAGE='Profile changed successfully';
const ERROR_SERVER_MESSAGE='Error occurred during the request. There may be a problem with connection or server is unavailable. Please wait a moment and try again';
const NOT_FOUND_MESSAGE='Not found';

// URL
const MOVIES_URL = 'https://api.nomoreparties.co';

// Movies
const SHORT_DURATION = 40;
const LARGE_WINDOW_SIZE = 1296;
const MEDIUM_WINDOW_SIZE = 497;
const SMALL_WINDOW_SIZE = 320;
const MOVIES_LARGE_WINDOW = 12;
const MOVIES_MEDIUM_WINDOW = 8;
const MOVIES_SMALL_WINDOW = 5;
const ADDED_MOVIES_LARGE_WINDOW = 3;
const ADDED_MOVIES_MEDIUM_WINDOW = 2;
const ADDED_MOVIES_SMALL_WINDOW = 2;

export {
    MAIN_ROUTE,
    MOVIES_ROUTE,
    SAVED_MOVIES_ROUTE,
    PROFILE_ROUTE,
    REGISTER_ROUTE,
    LOGIN_ROUTE,
    ERROR_MESSAGE,
    SUCCESS_LOGOUT_MESSAGE,
    UPDATE_PROFILE_MESSAGE,
    ERROR_SERVER_MESSAGE,
    NOT_FOUND_MESSAGE,
    MOVIES_URL,
    SHORT_DURATION,
    LARGE_WINDOW_SIZE,
    MEDIUM_WINDOW_SIZE,
    SMALL_WINDOW_SIZE,
    MOVIES_LARGE_WINDOW,
    MOVIES_MEDIUM_WINDOW,
    MOVIES_SMALL_WINDOW,
    ADDED_MOVIES_LARGE_WINDOW,
    ADDED_MOVIES_MEDIUM_WINDOW,
    ADDED_MOVIES_SMALL_WINDOW,
} 
